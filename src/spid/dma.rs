...
pub enum RX_FIFO_CLEAR_A {
    #[doc = "1: Clear the Receive FIFIO, clears any pending RX FIFO status."]
    CLEAR = 1,
}
...
impl<'a> RX_FIFO_CLEAR_W<'a> {
    ...
    #[doc = "Clear the Receive FIFIO, clears any pending RX FIFO status."]
    #[inline(always)]
    pub fn clear(self) -> &'a mut W {
...
impl R {
    #[doc = "Bits 0:5 - Transmit FIFO level that will trigger a DMA request, also level for threshold status. When TX FIFO has fewer than this many bytes, the associated events and conditions are triggered."]
    #[inline(always)]
    pub fn tx_fifo_level(&self) -> TX_FIFO_LEVEL_R {
        TX_FIFO_LEVEL_R::new((self.bits & 0x3f) as u8)
    }
    ...
    #[doc = "Bits 8:12 - Count of entries in TX FIFO."]
    #[inline(always)]
    ...
    #[doc = "Bits 16:21 - Receive FIFO level that will trigger a DMA request, also level for threshold status. When RX FIFO has more than this many bytes, the associated events and conditions are triggered."]
    #[inline(always)]
    pub fn rx_fifo_level(&self) -> RX_FIFO_LEVEL_R {
        RX_FIFO_LEVEL_R::new(((self.bits >> 16) & 0x3f) as u8)
    }
